...
export function Header() {
  ...
  useEffect(() => {
    ...
    const fetchUnreadCount = async () => {
      try {
        const response = await apiClient.get('/api/chat/dm/unread-count');
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center gap-4 h-16">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            {/* Message Icon - Right, before cart */}
            {isAuthenticated && (
              <Link 
                href="/chat" 
                className="relative flex-shrink-0 p-2 text-gray-700 hover:text-blue-600 transition-colors"
                title="Tin nhắn"
              >
